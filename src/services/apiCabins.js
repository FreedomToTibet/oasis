...
export async function createOrEditCabin(cabin, id) {
	const isHasImage = typeof cabin.image === 'string' && cabin.image.startsWith(supabaseUrl);
	// const imagePath = `${supabaseUrl}/storage/v1/object/public/cabins/${cabin.image[0].name}`;
...
export async function deleteCabin(id) {
  ...
  const { error: storageError } = await supabase.storage.from('cabins').remove([imageName]);
  ...
  if (error) {
	...
	// Check for and delete any entries in the storage with the value "undefined"
  const { data: undefinedEntries, error: undefinedEntriesError } = await supabase
    .storage
    .from('cabins')
    .list('', { limit: 20 });
  ...
  for (const entry of undefinedEntries) {
    if (entry.name === 'undefined') {
      const { error: deleteError } = await supabase.storage.from('cabins').remove([entry.name]);
      if (deleteError) {
